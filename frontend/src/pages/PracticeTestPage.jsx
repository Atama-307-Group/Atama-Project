...
const PracticeTestPage = ({ userId }) => {
  ...
  const handleSubmit = () => {
    if (currentQ.type === 'MCQ') {
      ...
      currentQ.correctAnswers.forEach((ans, i) => {
        ...
        const distance = getEditDistance(userInput, correctAns);

        // ADJUSTED LOGIC:
        // 1. Check distance
        // 2. Ensure user hasn't already dismissed this specific warning
        // 3. Removed the "correctAns.length > 2" block so "0." is recognized as close to "0"
        if (distance > 0 && distance <= 2 && !dismissedIndices.includes(i)) {
          newFuzzyIndices.push(i);
        }
      });

      if (newFuzzyIndices.length > 0) {
        setFuzzyIndices(newFuzzyIndices);
        return;
      }

      let correctCount = 0;
      currentQ.correctAnswers.forEach((ans, idx) => {
        if (userInputs[idx]?.trim().toLowerCase() === ans.toLowerCase()) {
          correctCount++;
        }
      });
  ...
  const resolveFuzzy = (idx, accept) => {
    if (accept) {
      ...
      // If user says "No", add to dismissed so we don't prompt again for this card
      ...
      <div style={{ maxWidth: '600px', margin: '50px auto', textAlign: 'center', fontFamily: 'sans-serif' }}>
                    ...
                    <div key={idx} style={{ width: '100%', position: 'relative' }}>
                      <input
                          ...
                          onChange={(e) => {
                            ...
                            setUserInputs(newInputs);
                            // If user types again, remove it from dismissed so they can be re-checked if they hit submit
                            if (dismissedIndices.includes(idx)) {
                              setDismissedIndices(prev => prev.filter(i => i !== idx));
                            }
                          }}
                          style={{
                            padding: '12px',
                      ...
                      {hasFuzzyWarning && (
                          <div style={{
                            ...
                            <div style={{ position: 'absolute', top: '-10px', left: '50%', transform: 'translateX(-50%)', width: 0, height: 0, borderLeft: '10px solid transparent', borderRight: '10px solid transparent', borderBottom: '10px solid #ffa500' }} />
        ...
        <div style={{ marginTop: '40px' }}>
          {!showFeedback ? (
              ...
              <button
                  onClick={handleNext}
                  style={{ padding: '12px 40px', fontSize: '18px', cursor: 'pointer', borderRadius: '8px', border: 'none', backgroundColor: '#007bff', color: '#fff' }}
              >
